...
`index.html` の `CONFIG.AI_MODE` で、AIを呼ぶ範囲を切り替えられます。

| 値 | 動作 | 用途 |
|---|---|---|
| `"fallback"` | キーワード不一致（そのほか）のときだけAI。緊急・繊細は監修文 | 既定・本番。最も安全 |
| `"safe"` | 緊急テーマ以外はすべてAI。緊急（urgent）は必ず監修文で確定 | AIを広く使いたい本番 |
| `"all"` | 緊急も含め、すべての入力をAIに回す | AIの挙動テスト専用 |

**AIをどんな入力でも試したいテスト中は `"all"` にします。** 本番に出すときは `"fallback"` か `"safe"` に戻してください（理由は下記のデメリット参照）。
